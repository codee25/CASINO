...
import aiohttp_cors # Додаємо для налаштування CORS
...
def update_user_balance(user_id, amount):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.query_fetchone() # Використовуємо query_fetchone
...
for route in list(app_aiohttp.router.routes()): # Робимо копію, бо маршрути змінюються під час ітерації
    # Додаємо умову для маршрутів, які потрібно обгорнути CORS.
    # Це можна зробити за назвами маршрутів або за їх шляхами, якщо назви не використовуються.
    if route.resource and route.resource.name in ['api_get_balance', 'api_spin']:
        cors.add(route)
    # Також можна додати CORS для інших маршрутів, якщо потрібно, наприклад, для Telegram webhook
    # if str(route.url_for()).startswith('/webhook/'):
    #     cors.add(route)
# =================================================================
...
# Важливо додавати 'name' до маршрутів, якщо ви використовуєте їх для ідентифікації в циклі CORS
